...
let inputQwerty = document.getElementById('qwerty')
let selectDead = document.getElementById('dead');
let selectEee = document.getElementById('eee');
...
function addTovar() {
    ...
    let product = {
        ...
        qwerty:         inputQwerty.value,
        dead:           selectDead.value,
        eee:            selectEee.value,
    };
...
function addTovarCard(tovar, index) {
    ...
    let card = `<div class="tovar-name">${ tovar.name }</div>
            ...
            <div class="tovar-place">Пункт выдачи: ${ tovar.qwerty }</div>
            <div class="tovar-dead">Цвет: ${ dead[tovar.dead] }</div>
            <div class="tovar-eee">Способ оплаты: ${ eee[tovar.eee] }</div>
            <div class="tovar-price-count">
                ${ price }
                <div class="count">Количество: ${ tovar.count } шт.</div>
            </div>
            <div class="tovar-close" onclick="closee(${ index })">X </div>
...
function edit(productIndex) {
    ...
    inputQwerty.value = product.qwerty
...
function closee(productIndex) {
    products.splice(productIndex, 1)
    console.log('close', productIndex)
